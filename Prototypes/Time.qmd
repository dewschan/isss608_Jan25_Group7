...
-   Across both variables, ARIMA consistently outperforms ETS, albeit slightly, on all three metrics.

-   This suggests that ARIMA may better capture the underlying time series patterns, especially in datasets that exhibit noise, irregular fluctuations, or subtle autocorrelations.

-   The performance gap, however, is not large, which means ETS remains a valid and practical alternative—particularly when simplicity, speed, or interpretability is more critical than slight improvements in accuracy.
